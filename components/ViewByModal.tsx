      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[85vh] flex flex-col">
        ...
        <div className="p-5 flex-1 overflow-auto">
          {loading ? (
            ...
            <p className="text-gray-500 text-center py-8">No {type === 'supplier' ? 'suppliers' : 'utilities'} found with client data.</p>
          ) : (
            <>
              ...
              <select
                ...
                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent mb-5"
              >
                {items.map((item) => (
                      ...
                      return Object.entries(byClient).map(([clientId, usages]) => (
                        <div key={clientId} className="border border-gray-200 rounded-lg overflow-hidden">
                          <Link
                            ...
                            className="block px-4 py-3 bg-gray-50 hover:bg-gray-100 font-medium text-gray-900"
                          >
                            {usages[0].clientName}
                          </Link>
                          <ul className="divide-y divide-gray-100">
                            {usages.map((u) => (
                              <li key={u.facilityId}>
                                <Link
                                  href={`/clients/${clientId}`}
                                  className="flex items-center px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 gap-2"
                                >
